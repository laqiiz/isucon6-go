...
func topHandler(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		...
		e.Stars = starMap[e.Keyword]
		entries = append(entries, &e)
	}
	rows.Close()
	log.Printf("select-entry term %v\n", time.Since(start1))
...
func loadStarMap() map[string][]*Star {
        ...
        for rows.Next() {
                ...
                err := rows.Scan(&s.ID, &s.Keyword, &s.UserName, &s.CreatedAt)
                panicIf(err)
                starMap[s.Keyword] = append(starMap[s.Keyword], s)
        }
        return starMap
}
